...
| T3        | 41–60   | A Convergência | Confirmação. O número 100 é real. O Observador começa a ser visto. | Só quem completou todos os 38 Habitantes de T1+T2 (19 por temporada: 16 fragmentos + 3 Âncoras) recebe o fragmento que confirma os 100 andares. |
...
- **T3:** Apenas quem completou TODOS os 38 Habitantes de T1+T2 (19 por temporada: 16 Habitantes base + 3 Âncoras, estrutura consistente entre temporadas) vê o fragmento especial que confirma os 100 andares. Para todos os outros é apenas mais um sussurro.
...
2. **Resistência à morte:** `primordial: true` no NPC (ver `lancamento.ts`). Mecânicas de morte existentes devem respeitar esse flag — o Primordial pode ser incapacitado mas não eliminado permanentemente por fadiga ou evento de processDay.
...
### O número 100
- Em T1, o número aparece de forma críptica em **apenas dois pontos**: no Sussurro IV · A Vigília (*"Os registros mais antigos mencionam uma centena. Não de andares. De algo que ainda não tem nome. A palavra foi apagada. O número permaneceu."*) e, como rumor condicionado, no `pioneers_fragment` ("cem câmaras" — grafia sancionada pela seção Livros).
- Em T2, referências ao número aumentam mas continuam ambíguas.
- Em T3, o fragmento especial (condicionado aos 38 Habitantes de T1+T2) confirma explicitamente: são 100 andares, e o Andar 100 é diferente de todos os outros.
...
- **T3:** O fragmento especial dos 38 Habitantes (T1+T2) inclui uma linha sobre O Observador. Não uma revelação — uma pergunta mais precisa.
...
- Backend para registrar quem chegou ao Andar 20 e em qual ordem (implementado: PioneerContext + `pioneer-api.ts`).
- Uma entrada especial no Codex com os 10 nomes — **dinâmica** (dados do backend), não em `CODEX_FRAGMENTOS` estático. Ainda a construir (ver seção 9).
- O `pioneers_fragment` estático em `CODEX_FRAGMENTOS` é outra coisa: o rumor "O Número Alterado" (cem câmaras), desbloqueado junto do marco dos Pioneiros. Não confundir os dois.
...
|------|-----------------------|--------------|
| "Eu esperei antes. Posso esperar de novo." | `lore-content.ts` → `PRIMORDIAIS_LORE.valdris.cardLore[3]` | Valdris tem memória de eras anteriores. Em T2, Thael vai reconhecê-lo. |
| "O que você é, afinal?" | `lore-content.ts` → `HABITANTES_LORE[4].falaConcluso` | O Observador tem uma natureza que não é humana e não é entidade. T4 responde parcialmente. |
| Referência críptica à centena | `lore-content.ts` → `SUSSURROS_LORE.sus_t4_4` (Sussurro IV · A Vigília) | O número 100 existe antes de qualquer texto explícito sobre andares. T3 confirma. |
| "A Torre não termina no vigésimo andar. Ela apenas... muda." | `lore-content.ts` → `VERDADES_LORE.verdade_t1` (frase final) | Preparação para T2 e além. Nenhuma temporada "termina" a Torre. |
| A mensagem que o destinatário era você | `lore-content.ts` → `HABITANTES_LORE[1].falaConcluso` | O Observador foi escolhido antes de chegar. Arco do Observador em T4. |
| O Ferreiro sabe que as correntes diminuem a cada andar | `lore-content.ts` → `HABITANTES_LORE[9].questLore` | Subir andares tem consequências. Preparação para o custo do ápice em T5. |
| Lista de nomes escrita antes de qualquer um nascer | `lore-content.ts` → `SUSSURROS_LORE.sus_t1_2` (Sussurro I · A Lista) | Destino vs. agência — tema central de T4 e T5. |
| "Quando os fragmentos convergirem, você o lerá também." | `lore-content.ts` → `PRIMORDIAIS_LORE.thael.cardLore[3]` | Thael leu "outro número" num registro apagado. Seed de T3 (A Convergência). |
...
2. **Cada Habitante é um fragmento do ser central — exceto as Âncoras.** Ao escrever novos Habitants para T2+, eles devem se encaixar na verdade revelada em `verdade_t1`: o ser foi dividido pelos Construtores e cada Habitante é uma parte. A exceção são as 3 Âncoras por temporada (andares 5/10/15, 25/30/35, …): plantadas pelo Fundador nos marcos de progressão, não nasceram do ser — mas a entidade aprendeu a absorvê-las também.
...
| Fragmento especial dos 38 Habitantes (confirmação dos 100 andares) | T3 | Todos os 38 Habitantes de T1+T2 (19 por temporada: 16 base + 3 Âncoras) implementados |
